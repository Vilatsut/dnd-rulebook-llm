...
model = ChatOpenAI(model=config.MODEL, base_url=config.MODEL_URL, api_key="not-needed")
...
print(
    f"Initializing vector store from directory {config.CHROMA_DB_PERSIST_DIRECTORY}..."
)
vector_store = Chroma(
